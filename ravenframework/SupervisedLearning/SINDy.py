...
class SINDy(SupervisedLearning):
  """
  ...
  def getInputSpecification(cls):
    ...
    spec.addSub(InputData.parameterInputFactory('degree',contentType=InputTypes.IntegerType,
                                                descr=r"""maximum number of iterations (generations) for the
                                                optimization problem  (differential evolution optimizer)""", default=2))
    return spec
  ...
  def _handleInput(self, paramInput):
    ...
    super()._handleInput(paramInput)
    settings, notFound = paramInput.findNodesAndExtractValues(['degree'])
    # notFound must be empty
    assert(not notFound)
    # self.degree =           # Pivot parameter ID
    # self.polyExpParams['expTerms']          = settings.get('numberExpTerms')         # the number of exponential terms
    # self.polyExpParams['coeffRegressor']    = settings.get('coeffRegressor') # which regressor to use for interpolating the coefficient
    # self.polyExpParams['polyOrder']         = settings.get('polyOrder')                # the polynomial order
    # self.polyExpParams['tol']               = settings.get('tol')                # optimization tolerance
    ...
    self.model = ps.SINDy(feature_library=ps.PolynomialLibrary(degree=settings.get("degree")))
  ...
  def __evaluateLocal__(self,featureVals):
    ...
    return {'y': self.model.predict(featureVals)}
